...
print('\n---Задача 4 - уровень normal - начало---\n')
# создаём список и заполняем его десятью произвольными целыми числами от 0 до 10
list = []
for element in range(10):
    list.append(random.randint(0, 10))
# через множества определяем уникальность значений и передаём их в новый список
newList = []
for element in set(list):
    newList.append(element)
# сформируем другой список из значений, которые в списке встречаются только один раз - будем проверять уникальные
lastList = []
for element in set(list):
    if list.count(element) == 1:
        lastList.append(element)

print('Исходный сформированный список был -\n{}\nа) новый список из уникальных элементов исходного списка:\n{}\n'
      'б) другой список из элеметов исходного списка, которые не имеют повторений:\n{}' .format(list, newList, lastList))

print('\n---Задача 4 - уровень normal - завершение---\n')
input('Ознакомьтесь с результатом и нажмите Enter')